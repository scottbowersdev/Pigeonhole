            ...
            <form class="max-w-screen-md m-auto" method="POST" action="/recurring-outgoings/{{ $recurring_outgoings->id }}">
                ...
                <div class="space-y-12">
                    <div class="border-b border-gray-900/10 pb-12">
                        <h2 class="text-base/7 font-semibold text-gray-900 dark:text-white">Edit Recurring Outgoing</h2>
                            ...
                            <div class="mt-10 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
                                <x-form.field>
                                    <div class="sm:col-span-5">
                                        ...
                                        <div class="mt-2">
                                            <x-form.input type="number" name="day" min="1" max="31" id="day" value="{{ $recurring_outgoings->day }}" placeholder="5" required />
                                ...
                                <div class="sm:col-span-5">
                                    ...
                                    <div class="mt-2">
                                        <div class="flex items-center rounded-md bg-white dark:bg-gray-900 pl-3 outline outline-1 -outline-offset-1 {{ $errors->has('cost') ? 'outline-red-500' : 'outline-gray-300 dark:outline-black' }} focus-within:outline focus-within:outline-2 focus-within:-outline-offset-2 focus-within:outline-indigo-600">
                                            ...
                                            <input type="number" min="0.01" step="0.01" name="cost" id="cost" value="{{ $recurring_outgoings->cost }}" class="block min-w-0 grow py-1.5 pl-1 px-3 text-base dark:text-white text-gray-900 dark:bg-slate-900 placeholder:text-gray-400 focus:outline focus:outline-0 sm:text-sm/6" placeholder="2000" required>
                                        </div>
                            ...
                            <div class="mt-10 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
                                <x-form.field>
                                    <div class="sm:col-span-5">
                                        ...
                                        <div class="mt-2">
                                            <x-form.input name="title" id="title" value="{{ $recurring_outgoings->title }}" placeholder="Car Payment" required />
                                    ...
                                    <select id="category" name="category" autocomplete="category" placeholder="-- Please Select --" class="col-start-1 row-start-1 w-full appearance-none rounded-md bg-white dark:bg-slate-900 dark:text-gray-100 py-1.5 pr-8 pl-3 text-base text-gray-900 outline-1 -outline-offset-1 {{ $errors->has('category') ? 'outline-red-500' : 'outline-gray-300' }} focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-600 sm:text-sm/6">
                                        ...
                                        @foreach($categories as $category)
                                            <option value="{{ $category->id }}"
                                                @if(old('category') == $category->id || $recurring_outgoings->categories->contains($category->id))
                                                    selected="selected"
                                                @endif
