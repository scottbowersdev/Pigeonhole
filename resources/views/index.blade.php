<x-layout>
    ...
    <ul role="list" class="divide-y divide-gray-100">
        ...
        <a href="/month/{{ $month['id'] }}" class="hover:text-white">
            <li class="flex justify-between gap-x-6 p-5 hover:bg-black hover:!text-white {{ ($loop->iteration % 2 == 0 ? 'bg-slate-200 dark:bg-slate-800' : FALSE) }}">
                <div class="flex min-w-0 gap-x-4">
                    <div class="min-w-0 flex-auto">
                        <p class="text-md/6 font-semibold dark:text-white">{{ date("F", mktime(0, 0, 0, $month['month'], 1)) }}</p>
                        <p class="mt-1 truncate text-xs/6 text-gray-500">{{ $month['year'] }}</p>
                    </div>
                </div>
                <div class="shrink-0 sm:flex sm:flex-col sm:items-end">
                    <p class="text-md/6 dark:text-gray-100">&pound;{{ number_format($month['income'],2) }}</p>
